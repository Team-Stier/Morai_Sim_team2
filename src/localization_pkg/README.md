...
개발용 `stop_required=true`는 유지한다. GPS 미수신 시간 제한은 2026-09-23 사용자 요청으로 제거했다.
...
GPS로 한 번 초기화된 뒤에는 GPS 미수신 시간 때문에 LOST로 전환하지 않는다.
15초/60초 제한과 위치 표준편차 3m 유효성 조건은 사용자 요청으로 제거했다.
GPS가 없으면 `DEAD_RECKONING`으로 계속 추정하며 LiDAR 벽 정합도 적용한다.
벽 정합 성공 여부와 공분산은 진단으로 보고하고 위치 유효성을 차단하지 않는다.
IMU 단절, 오래되거나 미래인 추정값, 시계 정지/역행, 초기화 부재는 여전히
무효다. 이 변경은 `stop_required=true`나 실제 추정 정확도를 바꾸지 않는다.
정책 원본은 중앙 timestamp 계약이다.
