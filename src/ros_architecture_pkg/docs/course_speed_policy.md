...
2026-09-21 최신 사용자 지시: 일반 구간 상한 58 km/h, 고주로 목표 순항 150 km/h.
앞선 일반 50·고주로 100 설정을 대체하며 고주로는 제한 해제를 유지한다.
원본 규정/지도 파일을 수정하지 않는다.
...
`null`, 그 외는 `58`이다. `null`은 제한 해제이며 목표 150과 다르다.
`speed_sections`는 표시용 연속 edge 구간이다. 원본 XYZ/MGeo max_speed는 보존한다.

Planner는 일반 목표 56 km/h, 고주로 목표 순항 150 km/h를 사용한다.
현재 제공 경로와 아래 감속·곡률 설정에서 계산한 계획 최고는 약 118.95 km/h다.
설정한 순항속도 도달이나 실제 속도의 상한 준수를 보장하는 것은 아니다.
...
최초 50/100 설정의 검증 기록: catkin 빌드, map/planner/controller/중앙 계약/bringup의
...
최초 50/100 설정의 실제 MORAI 관측: 허용된 GPS/IMU 기반 Localization으로 폐경로를 계속 추종했다.
...
58/150 변경 검증: 빌드·install, 변경 관련 지도/Planner/시각화 11개 검사와
지도·Planner·중앙 계약의 catkin 검사를 통과했다. 전체 누적 결과 집계는
314개 항목, 실패 0이다. HTML의 150/58 범례와 JavaScript 오류 부재,
RViz 수신 범례 `NO LIMIT | cruise 150 km/h`, `MAX 58 km/h`를 확인했다.
지도 RDDF 14개는 일반 제한 58/고주로 null 메타데이터로 재생성했다.
실제 실행에서 일반 구간 목표 56 km/h와 고주로 제어기 목표 약 118.6 km/h를
확인했다. 150 km/h 설정에 도달한 것으로 보고하지 않는다.
산출물은 `/home/paik/morai-artifacts/course-speed-58-150-20260921/`,
주행 원본은 `/home/paik/morai-artifacts/course-speed-58-150-live-observation.jsonl`이다.
