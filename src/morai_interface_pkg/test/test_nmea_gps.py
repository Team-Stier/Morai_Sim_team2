...
class SentencePolicyTest(unittest.TestCase):
    def test_gga_only_filters_rmc_and_keeps_gga(self):
        gga = nmea_gps.parse_nmea_sentence(
            nmea_gps.build_gga(37.0, 126.0, 10.0))
        rmc = nmea_gps.parse_nmea_sentence(
            nmea_gps.build_rmc(37.0, 126.0))
        self.assertTrue(nmea_gps.sentence_allowed(gga, "gga_only"))
        self.assertFalse(nmea_gps.sentence_allowed(rmc, "gga_only"))

    def test_all_policy_keeps_both_sentences(self):
        rmc = nmea_gps.parse_nmea_sentence(
            nmea_gps.build_rmc(37.0, 126.0))
        self.assertTrue(nmea_gps.sentence_allowed(rmc, "all"))

    def test_unknown_policy_is_rejected(self):
        gga = nmea_gps.parse_nmea_sentence(
            nmea_gps.build_gga(37.0, 126.0, 10.0))
        with self.assertRaises(ValueError):
            nmea_gps.sentence_allowed(gga, "prefer_rmc")
