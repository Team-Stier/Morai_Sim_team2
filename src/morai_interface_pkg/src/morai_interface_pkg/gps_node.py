...
인터페이스
- pub ~topic (기본 /sensors/gps/fix): sensor_msgs/NavSatFix
- 입력: MORAI GPS UDP(NMEA $GPGGA/$GPRMC), bind_ip:port로 수신

설계 메모
- timestamp: MORAI GPS UDP(NMEA)에는 완전한 절대 시각이 없어 수신 시각
  rospy.Time.now()를 header.stamp로 사용한다.
...
from morai_interface_pkg.protocol import nmea_gps
...
class MoraiGpsBridge(object):
    ...
    def __init__(self):
        ...
        self.topic = rospy.get_param("~topic", "/sensors/gps/fix")
        ...
        rospy.loginfo("[morai_gps_bridge] UDP %s:%d -> topic '%s' (frame_id=%s, "
                      "require_checksum=%s)", self.bind_ip, self.port, self.topic,
                      self.frame_id, self.require_checksum)
    ...
    def _log_stats(self, _event):
        ...
        rospy.loginfo("[morai_gps_bridge] recv=%d parse_ok=%d parse_fail=%d last_recv=%s",
