...
인터페이스
- pub ~topic (예 /sensors/camera/front/image/compressed): sensor_msgs/CompressedImage
...
- timestamp: ~timestamp_source = receive(수신시각) | packet(청크 sec/nsec). 기본 receive.
...
from morai_interface_pkg.protocol import camera_packet
...
class MoraiCameraBridge(object):
    ...
    def __init__(self):
        ...
        self.topic = rospy.get_param("~topic",
                                     "/sensors/camera/image/compressed")
    ...
    def spin(self):
        while not rospy.is_shutdown():
            ...
            if result is not None:
                ...
                self._publish(frame, sec, nsec)

    # 함수이름: _publish
    # 기능: 완성된 JPEG bytes를 CompressedImage로 발행한다(재인코딩 없음).
    def _publish(self, frame_bytes, sec, nsec):
    ...
    def _log_stats(self, _event):
        ...
        rospy.loginfo("[morai_camera_bridge] recv=%d completed=%d discarded=%d "
                      "box=%d parse_fail=%d last_recv=%s (last_discard=%s)",
