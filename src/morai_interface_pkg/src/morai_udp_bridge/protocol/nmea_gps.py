...
STATUS_SBAS_FIX = 1
STATUS_GBAS_FIX = 2
SUPPORTED_SENTENCE_POLICIES = ("gga_only", "all")
...
def sentence_allowed(fix, policy):
    """ROS 발행 대상으로 허용된 NMEA 문장인지 반환한다.

    MORAI는 같은 측정 epoch의 RMC와 GGA를 연속 송신한다. NavSatFix에는 고도와
    fix quality가 있는 GGA만 발행해 동일 위치의 중복 융합을 막는다.
    """
    if policy not in SUPPORTED_SENTENCE_POLICIES:
        raise ValueError("지원하지 않는 NMEA sentence_policy: %r" % policy)
    return policy == "all" or fix.sentence_type == "GGA"
