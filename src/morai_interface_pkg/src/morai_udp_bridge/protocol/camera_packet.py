...
class JpegFrameAssembler(object):
    ...
    def _begin(self, chunk, now):
        ...
        if self.check_chunk_index and chunk.index != 0:
            self._poisoned = True
            self._poison_reason = "missing_first_chunk"
    ...
    def add_chunk(self, chunk, now):
        ...
        # index=0은 새 프레임 시작이다. 이전 EI를 잃은 경우 기존 조각을 폐기하고
        # 새 프레임으로 재동기화한다.
        if self._started and self.check_chunk_index and chunk.index == 0:
            self._discard_current("missing_end_chunk")
        ...
        if self._started and chunk.is_end:
            ...
            if not frame.startswith(_JPEG_SOI) or not frame.endswith(_JPEG_EOI):
                self._discard_current("invalid_jpeg_markers")
                return None
