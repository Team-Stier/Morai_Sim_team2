...
- 역할: LiDAR는 자체 파서가 없고 velodyne_driver+pointcloud가 담당하므로,
        출력 토픽(/molit/sensors/lidar/points) 수신 여부만 감시해 상태를 노출한다.
- 주요 클래스: MoraiLidarWatchdog
인터페이스
- sub ~input_topic (기본 /molit/sensors/lidar/points): sensor_msgs/PointCloud2
- pub ~status_topic (기본 /molit/sensors/lidar/status): std_msgs/Bool (True=EXTERNAL_OK)
...
class MoraiLidarWatchdog(object):
    ...
    def _check(self, _event):
        ...
        stale = is_stale(now, reference, self.timeout_sec)
        ok = not stale
