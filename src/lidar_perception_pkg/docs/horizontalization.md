...
1. 원본 `lidar_link` scan + 같은 시각의 localization attitude, 센서 좌표 자차 제외.
2. roll/pitch 수평화. yaw를 0 방향으로 정렬하는 처리는 하지 않는다.
3. 수평 좌표에서 [구역별 지면 제거](ground_filter.md) 후 ROI X `[-20,50]`, Y `[-15,15]`, Z `[-1.5,1]` m 적용.
...
군집 후보가 된다. 차체 높이 변화·실제 경사로는 별도 지면 필터에서 다룬다.
현재 기본 DBSCAN은 수평화 후 구역별 지면 제거를 적용한다.
