...
class RouteSpeedProfileTest(unittest.TestCase):
    ...
    def test_150_cruise_policy_respects_curvature_braking_and_normal_limit(self):
        speeds = build_speed_profile(self.points, self.zones, 2, 2, 2, 15)
        self.assertEqual(self.zones.policy['high_speed']['cruise_kph'], 150)
        self.assertGreater(max(speeds)*3.6, 100)
        self.assertLessEqual(max(speeds)*3.6, 150)
        for i, speed in enumerate(speeds):
            self.assertGreater(speed, 0)
            if not self.zones.unlimited(i):
                self.assertLessEqual(speed*3.6, 56.000001)
        self.assertLessEqual(speeds[self.zones.end-1]*3.6, 56.000001)
        self.assertLess(min(speeds)*3.6, 30)  # Curve demand reduces cruise.

    def test_cyclic_deceleration_envelope_reaches_58_zone_before_entry(self):
        ...
        self.assertLessEqual(speeds[self.zones.end]*3.6, 56)
